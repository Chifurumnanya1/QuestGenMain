...
if st.button("🔍 Retrieve & Generate"):
    ...
    if st.button("📤 Send to Supabase"):
        ...
        with st.spinner("Uploading …"):
            try:
                r = requests.post(url,json=payload,headers=hdr,timeout=30); r.raise_for_status()
                st.success("🎉 Uploaded to Supabase!");  st.json(r.json() or {"status":"ok"})
            except requests.RequestException as e:
                st.error(f"RPC failed:\n{e}"); st.text(r.text if 'r' in locals() else "")
